"""Provider for locally installed LIRC infrared code databases."""
...
from .lirc_paths import resolve_lirc_database_path
from .lirc_reader import read_lirc_database
...
class LIRCProvider(CodeProvider):
    """Read and normalize every code in a locally installed LIRC database."""
    ...
    async def load(self, search_filter: SearchFilter | None = None) -> None:
        """Load LIRC codes once and apply the optional search filter."""
    ...
    async def iter_codes(
        ...
        """Yield LIRC codes that match the optional search filter."""
    ...
    def count(self) -> int:
        """Return the number of LIRC codes available after filtering."""
        return len(self._active_codes)

    def unfiltered_count(self) -> int:
        """Return the number of LIRC codes before filtering."""
        return len(self._cache or ())

    def reset(self) -> None:
        """Reset the LIRC cursor to its initial position."""
        self._index = 0

    def clear_cache(self) -> None:
        """Invalidate the cached LIRC database."""
        self._cache = None
        self._active_codes = []
        self._index = 0
    ...
    def _database_path(self) -> Path:
        if self._hass is None:
            raise RuntimeError("Home Assistant is required to locate the LIRC database")
        return resolve_lirc_database_path(self._hass)

    def _read_codes(self, database_path: Path) -> list[IRCode]:
        if not database_path.is_dir():
            _LOGGER.warning("LIRC database not found: %s", database_path)
            return []

        _LOGGER.debug("LIRC database found: %s", database_path)
        return read_lirc_database(database_path)

    def _apply_filter(self, search_filter: SearchFilter | None) -> None:
        """Apply the search filter to the cached LIRC codes."""
